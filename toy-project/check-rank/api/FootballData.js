...
class FootballData extends React.Component {
  constructor(token) {
  ...
  competitions({ area } = {}) {
    return this.instance.get(
      `/competitions?${querystring.stringify({ areas: area })}`
    );
  }
  ...
  teams(competitionId, { stage } = {}) {
    return this.instance.get(
      `/competitions/${competitionId}/teams?${querystring.stringify({ stage })}`
    );
  }
  ...
  competitionMatches(
    competitionId,
    { dateFrom, dateTo, stage, status, matchday, group } = {}
  ) {
    return this.instance.get(
      `/competitions/${competitionId}/matches?${querystring.stringify({
        dateFrom,
        dateTo,
        stage,
        status,
  ...
  teamMatches(id, { dateFrom, dateTo, status, venue } = {}) {
    return this.instance.get(
      `/teams/${id}/matches${querystring.stringify({
        dateFrom,
        dateTo,
        status,
        venue,
